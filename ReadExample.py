...
class DocExampleLoader(object):
            ...
            for tok in event.getElementsByTagName('token_anchor'):
                ...
                for xids,x in enumerate(curdoc.doc_seqs_ids):
                    if(tok_ids<x):
                        ...
                        aevent.event_tok_span.append((xids-1,tok_seq_ids))
                        break
    ...
    def parse_xml_get_relations_and_label(self,dom_tree,curdoc):
        ...
        for clink in Clinks:
            arelation=EventRelation()
            arelation.relation_id=clink.getAttribute('c-signalID')
            sources=clink.getElementsByTagName('source')
            targets=clink.getElementsByTagName('target')
            #暂且先假设一对关系只有一个s事件和一个e事件
            for source in sources:
                sid=int(source.getAttribute('id'))
                arelation.s_event=self.docid2eventid2event[curdoc.doc_id][sid]
